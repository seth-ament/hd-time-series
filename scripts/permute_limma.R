...
allele.notpermuted = meta$allele_nominal

sex = meta$sex

group = paste( strain , allele.notpermuted , sex , age.bins , sep="." )
...
degcounts.actual = colSums( decideTests( fit ) != 0 )


strata = paste( strain , sex , age.bins , sep="." )
...
n = nrow( meta )
ctrl = how( plots = Plots( strata = as.factor( strata ) ) )
perm = shuffle( n , ctrl )

allele = allele.notpermuted[ perm ]
...
return( colSums( decideTests( fit ) != 0 ) )

}

tab = do.call( rbind , degcounts )
tab = tab[,1:18]

apply( tab , 2 , quantile, probs = 0.95 )
summary( as.vector(tab) )
v = log10( as.vector(tab) )
v[ v == -Inf ] = 0

a = jitter( log10( degcounts.actual[1:18] ))
a[ a == -Inf ] = 0

pdf("permutation_deg_counts_distribution.pdf")
hist( v , breaks = 50 , ylim = c(-200,1700) , xlim = c(0,4) )
segments( 
   x0 = a ,
   y0 = -100 ,
   x1 = a ,
   y1 = 0 ,
   lwd = 4 , col = "maroon" )
#text( x = sort( a , decreasing = T )[1:3] , 
#   y = -15 ,
#   names( sort( a , decreasing = T )[1:3] ) )
dev.off()

save( tab , file="deg_counts_permuted_labels.RData" )
